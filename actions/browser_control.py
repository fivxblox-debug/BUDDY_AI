...
def _real_profile_dir(browser: str) -> str:
    ...
    print(
        f"[Browser] ⚠️ Real profile not found for {browser}, "
        f"using: {fallback}"
    )
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        print(
            f"[Browser] Firefox real profile: {default_path}"
        )
        return default_path
            ...
            for hive in (
                ...
                try:
                    ...
                    if exe and Path(exe).exists():
                        print(
                            f"[Browser] Opera found via registry: {exe}"
                        )
                        return exe
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            print(
                "[Browser] ⚠️ Opera executable not found on Windows."
            )
...
def _open_native(
    ...
    """
    Opens the user's real browser normally.
    """
    ...
    if name:
        ...
        if not exe and _OS == "Windows":
            if name in ("opera", "operagx"):
                ...
                exe = _find_exe_windows(
                    _WIN_EXE_HINTS.get(name, name)
                )
        ...
        if exe:
            try:
                ...
                print(
                    f"[Browser] Native launch failed "
                    f"for {name}: {e}"
                )

        print(
            f"[Browser] '{name}' not found — "
    ...
    try:
        ...
        try:
            if webbrowser.open(url):
                return (
                    f"Opened in your default browser: {url}"
                )
...
class _BrowserSession:
    ...
    def run(
        ...
        if not self._loop:
            raise RuntimeError(
                f"Session for '{self.browser_name}' "
                f"not started."
            )
    ...
    async def _adopt_page(self) -> Page:
        """
        launch_persistent_context already opens a startup tab.
        """
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                self._context = (
                    await engine_obj.launch_persistent_context(
                        profile,
                        **kwargs,
                    )
                )
                ...
                print(
                    "[Browser] Firefox real profile failed "
                    f"({e}), using JARVIS profile"
                )
        ...
        print(
            f"[Browser] Using dedicated automation profile: "
            f"{profile}"
        )
        ...
        try:
            ...
            print(
                f"[Browser] ⚠️ Real profile failed "
                f"for {label}: {e}"
            )
        ...
        Path(jarvis_profile).mkdir(
            parents=True,
            exist_ok=True,
        )

        print(
            f"[Browser] Retrying with JARVIS profile: "
            f"{jarvis_profile}"
        )
    ...
    async def go_to(self, url: str) -> str:
        ...
        async def _do_goto(p: Page) -> str:
            """
            Attempt navigation and return resulting URL.
            """
        ...
        if (
            ...
            print(
                "[Browser] Still blank after goto — "
                f"retrying on new tab: {url}"
            )
            ...
            try:
                ...
                print(
                    f"[Browser] New-tab retry failed: {e}"
                )

        if (
            result_url
            and result_url not in (
                "about:blank",
                "",
                None,
            )
    ...
    async def close_browser(self) -> str:
        ...
        return (
            f"{self.browser_name} closed."
        )


class _SessionRegistry:
    """Manage all active browser sessions."""
    ...
    def has(
        ...
        """
        Check whether this browser has an active
        automation session.
        """
    ...
    def pop_native_url(self) -> str:
        """
        Return and consume the last native URL.
        """

        url = self._last_native_url

        self._last_native_url = ""
    ...
    def close_one(
        ...
        if sess:
            ...
            return (
                f"{browser_name} closed."
            )

        return (
            f"No active session for: "
            f"{browser_name}"
        )
...
def browser_control(
    ...
    if action == "switch":
        ...
        result = (
            _registry.switch(target)
            if target
            else "Please specify a browser."
        )
    ...
    if action == "close":
        target = (
            browser
            or _registry._active_browser
        )

        result = (
            _registry.close_one(target)
            if target
            else "No browser specified."
        )
    ...
    # Navigation actions.
    # These use the native browser unless an
    # automation session already exists.
    ...
    if action in (
        ...
        if (
            result.startswith("Opened")
            and nav_url
        ):
            _registry.note_native_url(
                _normalize_url(nav_url)
            )
